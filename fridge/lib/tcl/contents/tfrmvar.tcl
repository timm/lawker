...
setQueueSize4 $rate $queue $node_(r1) $node_(r2)
...
for {set i 0} {$i < $flows} {incr i} {
    ...
    $ns duplex-link $node_(s$i) $node_(r1) 200Mb 2ms DropTail
    $ns duplex-link $node_(k$i) $node_(r2) 200Mb [expr $i/3]ms DropTail
    ...
    $ns duplex-link $node_(ts$i) $node_(r1) 200Mb 2ms DropTail
    $ns duplex-link $node_(tk$i) $node_(r2) 200Mb [expr $i/3]ms DropTail
