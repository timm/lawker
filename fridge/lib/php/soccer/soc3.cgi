...
BEGIN {
  ...
  print "<meta http-equiv='refresh' content='"DELAY";URL=soc3.cgi?" content "'>"
  ...
  print "&nbsp;&nbsp;&nbsp;<a href=soc3.cgi?init>init</a>"
  print "&nbsp;&nbsp;&nbsp;<a href=soc3.cgi?delay1=120&"content">2minpause</a>"
...
func maxkick(x,y,  z,zscale) {
  z = sqrt(x*x + y*y)
  if (z > 2*MAXDEVP) zscale = z/(2*MAXDEVP)
  else zscale = 1
  return x/zscale
}
...
func trykick(who) {
  ...
  if (!kicked) {
	...
	rrdev["0"] = rkick[who] + rand()*rrdev["0"]/20 - rand()*rrdev["0"]/20
	rrdev["0"] += rand()*rkick[who]/20 + rand()*rrdev[who]/20
	rrdev["0"] -= rand()*rkick[who]/20 + rand()*rrdev[who]/20
	ccdev["0"] = ckick[who] + rand()*ccdev["0"]/20 - rand()*ccdev["0"]/20
