...
# to do:  scale maxdev and maxkick properly
  ...
  print "<meta http-equiv='refresh' content='"DELAY";URL=soc2.cgi?" content "'>"
  ...
  print "&nbsp;&nbsp;&nbsp;<a href=soc2.cgi?init>init</a>"
  print "&nbsp;&nbsp;&nbsp;<a href=soc2.cgi?delay1=120&"content">2minpause</a>"
  ...
  for (i in dat) {
    graph(rdat[i],cdat[i],"<font size=-3>"symbol(i) printme(i)"</font>",color[i])
  }
  ...
  # print "writing"
  for (i in dat) {
    if (i == "0") print i, color[i], rdat[i], cdat[i], edat[i], notes[i], "static"static[i] > "pos"
    else print i, color[i], rdat[i], cdat[i], edat[i], notes[i], static[i] > "pos"
  }
  close("pos")
  for (i in color) {
    if (i == "0") print i, color[i], rrdev[i], ccdev[i], kick[i] > "dirs"
    else print i, color[i], rdev[i], cdev[i], kick[i] > "dirs"
  }
  close("dirs")
  ...
  for (i in dat) {
    ...
    if (i != "0" && i !="a" && i != "A") {
      if (edat[i] > 20 && chaser[i] || (abs(cdat["0"]-cdat[i]) < 50 && abs(rdat["0"]-rdat[i]) < 20)) {
        ...
        if (edat[i] > 20 && abs(cdat["0"]-cdat[i]) < 4 && abs(rdat["0"]-rdat[i]) < 4) {
...
func maxkick(x) {
  if (x > 2.5*MAXDEVP) return 2.5*MAXDEVP
  if (x < -2.5*MAXDEVP) return -2.5*MAXDEVP
  return x
}
...
func trykick(who) {
  ...
  if (!kicked) {
	...
	rrdev["0"] = rkick[who] + rand()*rrdev["0"]/10 - rand()*rrdev["0"]/10
	rrdev["0"] += rand()*rkick[who]/10 + rand()*rrdev[who]/10
	rrdev["0"] -= rand()*rkick[who]/10 + rand()*rrdev[who]/10
	ccdev["0"] = ckick[who] + rand()*ccdev["0"]/10 - rand()*ccdev["0"]/10
	ccdev["0"] += rand()*ckick[who]/10 + rand()*ccdev[who]/10
	ccdev["0"] -= rand()*ckick[who]/10 + rand()*ccdev[who]/10
	static["0"] = 0
	kicked = 1
	if (dirdat[who] ~ /aloft/) {
