#!/usr/local/bin/gawk -f 
 ...
 # All rights reserved. Licensed under the GNU Public license (GPL).

BEGIN {
	print "#### translated by awk++ alpha 20"
	TRUE = 1
	FALSE = 0
	RS="\r\r\n"
	FS="\r\r\n"
	
	# regular expressions are defined at the end of this file
	defineREs()
}

{
	# The whole program is read in at once due to the RS setting
	compiled = compileProg($0)
}
...
function appendClassSelector(compiled) {
	...
	compiled = compiled  "function __CallMethod(object,method,p1,p2,p3,p4,p5,p6,p7,p8,p9, __results) {\n"
	compiled = compiled "if(method ==\"delete\") __DestroyObject(object);\n"
	for(i in classes) 
		compiled = compiled  "else if(__objects[object] == \"" i "\") __results = " i "(object,method,p1,p2,p3,p4,p5,p6,p7,p8,p9);\n"
...
function appendUtilityFunctions(compiled) {
	...
	compiled = compiled  "function __ConstructObject(class,p1,p2,p3,p4,p5,p6,p7,p8,p9) {\n"
	...
	compiled = compiled  "    __CallMethod(__ObjectNumber,\"new\",p1,p2,p3,p4,p5,p6,p7,p8,p9) \n"
...
function classDef(classFound,program,arrcc,  n,i,__work,__inheritedClasses,currentClass,retainBegin,retainEnd) {
	# input contains:
	...
	#	the class function with inherited class code at the end
			...
			if(__work[5]) {
				...
				for(i=2;i<=n;i++) {
					program = __inheritedClasses[i] "(this,method,p1,p2,p3,p4,p5,p6,p7,p8,p9); " program                    # "construct" superclasses
					program = program __inheritedClasses[i] "(this,method,p1,p2,p3,p4,p5,p6,p7,p8,p9) "                    # pass unidentified method to superclasses
				}
				program = " if(method == \"new\") {" program  
				program = program ")}"
			}
			program = __work[1] "function " currentClass \
				"(this,method,p1,p2,p3,p4,p5,p6,p7,p8,p9) " __work[7] program "}"
			return program
}
